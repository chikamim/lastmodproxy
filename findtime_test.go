...
func TestFindLastModified(t *testing.T) {
	body := `Title
Edited at 2013-12-10 12:14
posted at 2013-12-09 12:45
最終更新: 2013/12/10 12:14
Body
`
	re := regexp.MustCompile(`最終更新: (\d{4}/\d{2}/\d{2} \d{2}:\d{2})`)
	actual, err := FindTime([]byte(body), re, "2006/01/02 15:04", time.Local)
	if err != nil {
		t.Errorf("error occurred - %v", err)
	}
	expected := time.Date(2013, time.December, 10, 12, 14, 0, 0, time.Local)
	if actual != expected {
